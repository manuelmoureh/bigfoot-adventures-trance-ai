...
| SEC-4 | `npm audit`: critical unauthenticated RCE in Next.js 16.0.0-16.3.2 (we were on 16.3.1), including a Windows-hosted-server-specific vector — a live risk on this Windows dev machine, not just a deployed-server concern. | **P0 (fixed)** | Patched to 16.3.5 via `npm audit fix --force` (non-breaking patch release despite the flag) — see `Audit/RESULTS.md`. `npm audit` now reports 0 vulnerabilities. |
